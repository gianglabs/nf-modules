...
An nf-core modules repository hosting Nextflow DSL2 modules for the g-labs organization.
...
2. List the available modules:
   ...
   ```bash
   nf-core modules --git-remote https://github.com/g-labs/modules.git list remote
   ```
   ...
   INFO     Modules available from https://github.com/g-labs/modules.git (main):
...
3. Install the module in your pipeline directory:
   ...
   ```bash
   nf-core modules --git-remote https://github.com/g-labs/modules.git install examplemodule
   ```
   ...
   INFO     Use the following statement to include this module:
    ...
    include { EXAMPLEMODULE } from '../modules/g-labs/examplemodule/main'
   ```
...
5. Check that a locally installed g-labs module is up-to-date compared to the one hosted in this repo:

   ```bash
   nf-core modules --git-remote https://github.com/g-labs/modules.git lint examplemodule
   ```
   ...
   │╶─────────────┼─────────────────────────────────────┼────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────╴│
   │ examplemodule     │ modules/g-labs/examplemodule/main.nf │ Unable to connect to container registry, code:  404, url: https://community.wave.seqera.io/library/seqtk_perl:37201934bb74266e          │
   │ examplemodule     │ modules/g-labs/examplemodule/main.nf │ Container versions do not match                                                                                                         │
   │              ╵                                     ╵                                                                                                                                         │
   ╰──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────╯
   ╭───────────────────────╮
...
6. Remove the module from the pipeline repository if required:
   ...
   ```bash
   nf-core modules --git-remote https://github.com/g-labs/modules.git remove examplemodule
   ```
...
1. List the available sub-workflows:
   ...
   ```bash
   nf-core subworkflows --git-remote https://github.com/g-labs/modules.git list remote
   ```
   ...
   INFO     Subworkflows available from https://github.com/g-labs/modules.git (main):
...
2. Install the sub-workflow in your pipeline directory:
   ...
   ```bash
   nf-core subworkflows --git-remote https://github.com/g-labs/modules.git install examplesubworkflow
   ```
   ...
   INFO     Use the following statement to include this subworkflow:
    ...
    include { EXAMPLESUBWORKFLOW } from '../subworkflows/g-labs/examplesubworkflow/main'
   ```
...
4. Check that a locally installed g-labs sub-workflow is up-to-date compared to the one hosted in this repo:

   ```bash
   nf-core subworkflows --git-remote https://github.com/g-labs/modules.git lint examplesubworkflow
   ```
...
5. Remove the sub-workflow from the pipeline repository if required:
   ...
   ```bash
   nf-core subworkflows --git-remote https://github.com/g-labs/modules.git remove examplesubworkflow
   ```
...
"Cross-organisation" sub-workflows are sub-workflows that contain components from both `nf-core/modules` and `g-labs/modules`.
...
1. Write sub-workflows `.nf` files that refer to locations in both `g-labs` and `nf-core`.
...
2. In `meta.yml`:
   1. Change the first line to
      ...
      ```yaml
      # yaml-language-server: $schema=https://raw.githubusercontent.com/g-labs/modules/main/subworkflows/yaml-schema.json
      ```
...
1. In `.pre-commit-config.yaml`, add extra lines to ignore g-labs modules and sub-workflows the same way nf-core ones are ignored:
...
2. `nf-test.config` needs similar rules:
   ...
   +    ignore 'modules/nf-core/**/*', 'subworkflows/nf-core/**/*', 'modules/g-labs/**/*', 'subworkflows/g-labs/**/*'
...
3. So does `.gitattributes` (to automatically collapse files changed in the sanger-tol components)
   ...
   *.nf.test linguist-language=nextflow
   ...
   +modules/g-labs/** linguist-generated
   +subworkflows/g-labs/** linguist-generated
   ```
...
```bash
copier copy --vcs-ref main gh:nf-core/modules-template ./modules
```
